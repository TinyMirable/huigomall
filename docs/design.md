...
```
hgmall 
├── hgmall-common -- 工具类及通用代码 （编写部分）
├── hgmall-mbg -- MyBatisGenerator生成的数据库操作代码 （编写部分）
├── hgmall-security -- SpringSecurity封装公用模块 （未实现）
├── hgmall-admin -- 后台商城管理系统接口 （实现部分）
├── hgmall-search -- 商品搜索系统 (未实现)
├── hgmall-portal -- 用户相关商城接口 （实现部分）
└── hgmall-web --前端vite代码
...
#### 2.1.4 商品状态（ProductStatus）

| 状态码 | 常量名 | 说明 | 业务影响 |
|--------|--------|------|----------|
| 1 | ON_SHELF | 上架 | 商品正常在售，用户可以购买 |
| 0 | OFF_SHELF | 下架 | 商家主动下架，用户无法购买，商家可以重新上架 |
| 2 | ADMIN_OFF_SHELF | 管理员下架 | 管理员强制下架，用户无法购买，商家无法重新上架，需要管理员操作 |
...
- **请求体**:
  ...
  ```
- **功能**: 发送验证码（通用接口，用于用户操作），需要验证用户存在（用于忘记密码、修改信息等场景）
- **权限**: 无需认证（公开接口）

#### 4.2.2 发送注册验证码
- **接口路径**: `POST /api/auth/send-register-code`
- **请求体**:
  ```json
  {
    "contact": "user@example.com"  // 邮箱或手机号
  ...
  ```
- **功能**: 发送注册验证码，需要验证用户不存在（仅用于注册）
- **权限**: 无需认证（公开接口）

#### 4.2.3 发送登录验证码
- **接口路径**: `POST /api/auth/send-login-code`
- **请求体**:
  ```json
  {
    "contact": "user@example.com"  // 邮箱或手机号
...
- **请求体**:
  ```json
  {
    "identifier": "user",  // 用户名/邮箱/手机号
    "password": "password"  // 密码
  }
  ```
- **功能**: 支持用户名/邮箱/手机号+密码登录
- **返回内容**: 登录结果（包含token）
- **权限**: 无需认证（公开接口）

#### 4.2.5 用户验证码登录
- **接口路径**: `POST /api/auth/login-by-code`
- **请求体**:
  ```json
  {
    "contact": "user@example.com",  // 邮箱或手机号
...
#### 4.2.9 用户注册为商家
- **接口路径**: `POST /api/auth/register-merchant`
- **请求体**:
  ```json
  {
    "merchantName": "商家名称",
  ...
  ```
- **功能**: 用户注册为商家，需要用户已登录
- **权限**: 需要用户身份验证（需要Authorization token）

### 4.3 用户信息管理接口（hgmall-portal）

#### 4.3.1 获取当前用户概要信息
- **接口路径**: `GET /api/users/me/summary`
- **返回内容**: 用户概要信息（用户名、邮箱、手机号等）
- **权限**: 需要用户身份验证（需要Authorization token）

#### 4.3.2 修改用户名
- **接口路径**: `POST /api/users/me/username`
- **请求体**:
  ```json
  {
    "username": "新用户名"
...
#### 4.3.3 发送邮箱验证码（用于修改邮箱）
- **接口路径**: `POST /api/users/me/email/send-code`
- **请求体**:
  ```json
  {
  ...
  ```
- **功能**: 发送验证码到新邮箱（用于修改邮箱操作）
- **权限**: 需要用户身份验证（需要Authorization token）

#### 4.3.4 修改邮箱
- **接口路径**: `PUT /api/users/me/email`
- **请求体**:
  ```json
  {
    "email": "newemail@example.com",  // 新邮箱
    "code": "123456"  // 验证码（普通用户和商家需要，管理员不需要）
  }
  ```
- **功能**: 修改当前用户的邮箱，普通用户和商家需要验证码，管理员不需要
- **权限**: 需要用户身份验证（需要Authorization token）

#### 4.3.5 修改手机号
- **接口路径**: `PUT /api/users/me/phone`
- **请求体**:
  ```json
  {
    "phoneNumber": "13800138000",  // 新手机号
    "code": "123456"  // 邮箱验证码（如果用户没有邮箱，提示先设置邮箱，管理员不需要）
  }
  ```
- **功能**: 修改当前用户的手机号，普通用户和商家需要邮箱验证码（如果用户没有邮箱，提示先设置邮箱），管理员不需要
- **权限**: 需要用户身份验证（需要Authorization token）

### 4.4 地址管理接口（hgmall-portal）
...
#### 4.4.2 获取地址详情
- **接口路径**: `GET /api/addresses/{addressId}`
- **返回内容**: 地址详细信息
- **权限**: 需要用户身份验证（需要Authorization token），只能访问自己的地址
...
#### 4.4.4 更新地址
- **接口路径**: `PUT /api/addresses/{addressId}`
- **请求体**:
  ```json
  {
    "detail": "详细地址",
...
- **功能**: 更新地址信息
- **返回内容**: 更新后的地址信息
- **权限**: 需要用户身份验证（需要Authorization token），只能更新自己的地址

#### 4.4.5 删除地址
- **接口路径**: `DELETE /api/addresses/{addressId}`
- **功能**: 删除地址
- **权限**: 需要用户身份验证（需要Authorization token），只能删除自己的地址

#### 4.4.6 设置默认地址
- **接口路径**: `PUT /api/addresses/{addressId}/set-default`
- **功能**: 将指定地址设置为默认地址
- **返回内容**: 更新后的地址信息
- **权限**: 需要用户身份验证（需要Authorization token），只能设置自己的地址为默认地址
...
##### 0.2 管理员验证码登录
- **接口路径**: `POST /api/admin/auth/login-by-code`
- **请求体**:
  ```json
  {
    "contact": "admin@example.com",  // 邮箱或手机号
    "code": "123456"  // 验证码
  }
  ```
- **功能**: 支持邮箱/手机号+验证码登录
...
##### 0.3 发送管理员登录验证码
- **接口路径**: `POST /api/admin/auth/send-login-code`
- **请求体**:
  ```json
  {
    "contact": "admin@example.com"  // 邮箱或手机号
  ...
  ```
- **功能**: 发送管理员登录验证码，发送前会验证用户是否存在且为管理员
- **权限**: 无需认证（公开接口）

##### 0.4 发送验证码（通用）
- **接口路径**: `POST /api/admin/auth/send-code`
- **请求体**:
  ```json
  {
    "contact": "admin@example.com"  // 邮箱或手机号
  ...
  ```
- **功能**: 发送验证码（通用接口，用于管理员操作），需要验证用户存在且为管理员
- **权限**: 无需认证（公开接口）
...
- **请求体**:
  ```json
  {
    "parentId": 0,  // 0或null表示一级分类，否则为父分类ID
    "name": "电子产品"
  }
  ```
- **功能**: 创建商品分类
- **权限**: 需要管理员身份验证
- **审计日志**: 记录操作日志
- **说明**: `sortOrder` 参数已废弃，优先级会自动根据分类级别生成
...
- **请求体**:
  ...
  {
    "name": "电子产品（更新）",
    "parentId": 1  // 可选，可修改父分类ID
  }
  ```
- **功能**: 修改分类信息（可修改分类名称和父分类）
- **权限**: 需要管理员身份验证
- **审计日志**: 记录操作日志
- **说明**: `sortOrder` 参数已废弃，优先级会自动根据分类级别生成
...
##### 8.1 查看所有店铺列表
- **接口路径**: `GET /api/admin/shops`
- **请求参数**:
  - `status`(可选): 店铺状态筛选（1-正常，2-关闭，3-管理员强制关闭）
  - `keyword`(可选): 关键词搜索（店铺名称）
  - `page`(可选，默认1): 页码
  - `size`(可选，默认20): 每页大小
- **返回内容**: 店铺列表（包含店铺信息、商家信息等）
- **权限**: 需要管理员身份验证

##### 8.2 修改店铺状态
- **接口路径**: `PUT /api/admin/shops/{shopId}/status`
- **请求参数**:
  - `status`(必填): 新状态（1-正常，2-关闭，3-管理员强制关闭）
- **功能**: 管理员修改店铺状态
- **权限**: 需要管理员身份验证
- **审计日志**: 记录操作日志
...
##### 9.1 修改商品状态
- **接口路径**: `PUT /api/admin/products/{productId}/status`
- **请求参数**:
  - `status`(必填): 新状态（1-上架，0-下架，2-管理员强制下架）
- **功能**: 管理员修改商品状态（商品上下架），管理员可以将商品设置为上架、下架或强制下架状态
- **权限**: 需要管理员身份验证
- **审计日志**: 记录操作日志
